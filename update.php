...
if ($state_ok && (!isset($_SESSION['auto_update_new']) || $_SESSION['auto_update_new'] === 2)) {
	echo '<p class="new-version-update-missing">New NVLL version is undetermined, please try again.</p>';
	unset($_SESSION['auto_update_new']);
	$state_ok = false;
}

if ($state_ok && (!isset($_SESSION['auto_update_new']) || $_SESSION['auto_update_new'] === 1)) {
	echo '<p class="new-version-update-missing">No new NVLL version.</p>';
	$state_ok = false;
}
...
if ($state_ok) {
	...
	if (
		...
		echo '<p class="new-version-update-missing">No update possible. Current version: ' . $conf->nvll_version . ', new version: ' . $new_version . '</p>';
		unset($_SESSION['auto_update_new']);
		$state_ok = false;
	}
}
...
if ($state_ok) {
	echo '<h2 class="new-version-update-info">Your version is: ' . $conf->nvll_version . ', Updating to version: ' . $new_version . '</h2>';
	...
	if (is_writable(".")) {
		echo '<p class="new-version-update-ok">NVLL root directory is writable.</p>';
	} else {
		echo '<p class="new-version-update-missing">NVLL root directory is not writable.</p>';
		$state_ok = false;
	}
...
if (!isset($_GET['doUpdate']) || $_GET['doUpdate'] != 1) {
	if ($state_ok) {
		...
		if (strncasecmp(PHP_OS, "win", 3) == 0) {
			$archive_name = "nvll-" . $conf->nvll_version . "_backup_" . date("YmdHis") . ".zip";
		} else {
			$archive_name = "nvll-" . $conf->nvll_version . "_backup_" . date("YmdHis") . ".tar.gz";
		}
		//unpacking zip file with Phar does not work on windows
		if (strncasecmp(PHP_OS, "win", 3) == 0) {
			$download_link = 'http://downloads.sourceforge.net/project/nvll/NVLL/' . $new_version . '/nvll-' . $new_version . '.zip';
		} else {
			$download_link = 'http://downloads.sourceforge.net/project/nvll/NVLL/' . $new_version . '/nvll-' . $new_version . '.tar.gz';
		}
		echo '<h2 class="new-version-update-info"><a href="update.php?' . NVLL_Session::getUrlGetSession() . '&doUpdate=1">Click here to automatically backup and update your existing NVLL installation. ONLY CLICK ONCE AND WAIT!</a></h2>';
		...
		echo '<li class="new-version-update-info">Your old NVLL files will be saved as archive in your NVLL root directory.</li>';
		...
		echo '<li class="new-version-update-info">If you need to go back just unpack the archive in your NVLL root directory overwriting the existing files.</li>';
		echo '</ul>';
		echo '<h4 class="new-version-update-info">Update:</h3>';
		echo '<ul class="new-version-update-info">';
		echo '<li class="new-version-update-info">The latest NVLL version will be downloaded automatically from the projects web site: <a href="' . $download_link . '">' . $download_link . '</a></li>';
	...
	error_log('NVLL: user <' . $_SESSION['nvll_user'] . '@' . $_SESSION['nvll_domain'] . '> initiated system update');
	if ($state_ok) {
		if ($nvll_update_running) {
			$state_ok = false;
		} else {
			$step = 0;
			file_put_contents("NVLL_UPDATE_RUNNING", $step);
		}
	}
	if ($state_ok) {
		...
		try {
			...
			if (strncasecmp(PHP_OS, "win", 3) == 0) {
				...
				$archive_name = "nvll-" . $conf->nvll_version . "_backup_" . date("YmdHis") . ".tar";
				echo '<p class="new-version-update-info">Creating backup file: ' . $archive_name . '</p>';
	...
	if ($state_ok) {
		...
		//unpacking zip file with Phar does not work on windows
		if (strncasecmp(PHP_OS, "win", 3) == 0) {
			$archive_name = 'nvll-' . $new_version . '.zip';
			$download_link = 'http://downloads.sourceforge.net/project/nvll/NVLL/' . $new_version . '/nvll-' . $new_version . '.zip';
		} else {
	...
	if ($state_ok) {
		...
		$ckecksums = file_get_contents('https://nvll.sourceforge.net/checksums.txt');
		$checksum = "";
		$checksumType = "";
		$matches = array();
		if (preg_match("/" . $archive_name . "\tsha256\t(\S+)\R/", $ckecksums, $matches)) {
			...
			echo '<p class="new-version-update-missing">Could not find checksum. See also <a href="http://nvll.sourceforge.net/download/" target="_blank">http://nvll.sourceforge.net/download/</a></p>';
			$state_ok = false;
		}
		if ($checksumType == "md5") {
			$md5sum_check = md5_file($archive_name);
			if ($checksum == $md5sum_check) {
				echo '<p class="new-version-update-ok">MD5 checksums match: ' . $checksum . '. See also <a href="http://nvll.sourceforge.net/download/" target="_blank">http://nvll.sourceforge.net/download/</a></p>';
			} else {
				echo '<p class="new-version-update-missing">Failed md5 checksum match: ' . $md5sum_check . ' should be ' . $checksum . '.</p>';
				echo '<p class="new-version-update-missing">See also <a href="http://nvll.sourceforge.net/download/" target="_blank">http://nvll.sourceforge.net/download/</a></p>';
				$state_ok = false;
			}
		}
		if ($checksumType == "sha256") {
			...
			if ($checksum == $sha256sum_check) {
				echo '<p class="new-version-update-ok">sha256 checksums match: ' . $checksum . '. See also <a href="http://nvll.sourceforge.net/download/" target="_blank">http://nvll.sourceforge.net/download/</a></p>';
			} else {
			...
			if (preg_match("/\.htaccess/", $file)) {
				if (is_file($file)) {
					...
					if ($new_htaccess != $old_htaccess) {
						...
						echo '&nbsp;&nbsp;Integrate content of new file ' . $file . ".nvll-" . $new_version . ' into ' . $file . '</p>';
					} else {
