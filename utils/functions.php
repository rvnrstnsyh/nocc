...
function buildfolderlink($folder)
{
    ...
    for ($i = 0; $i < count($elements); $i++) {
        ...
        echo "<a href=\"api.php?" . NVLL_Session::getUrlGetSession() . "&folder=" . $folderpath . "\">" . $elements[$i] . "</a>";
    }
    echo "\n";
}
...
function get_page_nav($pages, $skip)
{
    ...
    if ($pages > 1) { // if there several pages...
        ...
        $html = '<form method="POST" action="api.php?' . NVLL_Session::getUrlGetSession() . '">';
        ...
        if ($pskip > -1) // if NOT first page...
            $html .= '<li class="prev"><a href="api.php?' . NVLL_Session::getUrlGetSession() . '&skip=' . $pskip . '" title="' . $title_prev_page . '" rel="prev">&laquo; ' . $alt_prev . '</a></li>';
        else // if first page...
            $html .= '<li class="prev"><span> &laquo; ' . $alt_prev . '</span></li>';
        if ($start_page > 1) {
            $html .= '<li class="page"><a href="api.php?' . NVLL_Session::getUrlGetSession() . '&skip=0" title="' . $html_page . ' 1" rel="first">1</a></li>';
            if ($start_page > 2) {
        ...
        for ($xpage = $start_page; $xpage <= $end_page; $xpage++) { // for all visible pages...
            ...
            if ($xpage == $page) // if current page...
                ...
                $html .= '<li class="page"><a href="api.php?' . NVLL_Session::getUrlGetSession() . '&skip=' . $xskip . '" title="' . $html_page . ' ' . $xpage . '">' . $xpage . '</a></li>';
        }
        if ($end_page < $pages) {
            if ($end_page < $pages - 1) {
                $html .= '<li class="extend"><span>&hellip;</span></li>';
            }
            $html .= '<li class="page"><a href="api.php?' . NVLL_Session::getUrlGetSession() . '&skip=' . ($pages - 1) . '" title="' . $html_page . ' ' . $pages . '" rel="last">' . $pages . '</a></li>';
        }
        if ($nskip < $pages) // if NOT last page...
            $html .= '<li class="next"><a href="api.php?' . NVLL_Session::getUrlGetSession() . '&skip=' . $nskip . '" title="' . $title_next_page . '" rel="next">' . $alt_next . ' &raquo;</a></li>';
        else // if last page...
