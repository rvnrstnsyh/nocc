...
{
	global $conf;
	if (!NOCC_Session::existsUserPrefs()) return '';
	...
	if (!$mailAddress->hasAddress()) {
		if (isset($_SESSION['nocc_login_mailaddress'])) {
			...
			if (isset($_SESSION['nocc_login']) && strlen($_SESSION['nocc_login']) > 0 && isset($_SESSION['nocc_domain']) && strlen($_SESSION['nocc_domain']) > 0) {
				$user_part = $_SESSION['nocc_login'];

				if (isset($conf->domains[$_SESSION['nocc_domainnum']]->from_part) && strlen($conf->domains[$_SESSION['nocc_domainnum']]->from_part) > 0) {
